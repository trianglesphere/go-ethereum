...
func (ethash *Ethash) Finalize(ctx context.Context, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) {
	span, ctx := opentracing.StartSpanFromContext(ctx, "finalize")
	defer span.Finish()
...
func (ethash *Ethash) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	// Finalize block
	ethash.Finalize(context.TODO(), chain, header, state, txs, uncles)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	for _, uncle := range uncles {
		...
		r.Mul(r, blockReward)
		r.Div(r, big8)
		state.AddBalance(context.TODO(), uncle.Coinbase, r)

		r.Div(blockReward, big32)
		reward.Add(reward, r)
	}
	state.AddBalance(context.TODO(), header.Coinbase, reward)
}
